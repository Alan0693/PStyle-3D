...
# modify
@click.option('--outdir', help='Where to save the out images', type=str, required=False, metavar='DIR', default='out', show_default=True)
@click.option('--network', 'network_pkl', help='Network pickle filename', type=str, required=False, metavar='DIR', default='/data1/sch/EG3D-projector-master/eg3d/networks/ffhq512-128.pth', show_default=True)
...
def generate_images(
    ...
    if reload_modules:
        ...
        rendering_kwargs = {'depth_resolution': 96, 'depth_resolution_importance': 96, 'ray_start': 2.25,
                            ...
                            'sr_antialias': True}
        G = TriPlaneGenerator(*init_args, **init_kwargs).requires_grad_(False).to(device)

        ckpt = torch.load('/data1/sch/EG3D-projector-master/eg3d/networks/ffhq_stdcrop-128.pth')
    ...
    #         # depth_img = G.synthesis(ws, camera_params)['image_depth']

    #         img = (img.permute(0, 2, 3, 1) * 127.5 + 128).clamp(0, 255).to(torch.uint8)
    #         # depth_img = depth_img.permute(0, 2, 3, 1).to(torch.uint8).squeeze(3)
    #         # depth_img = depth_img[0].cpu().numpy()
    #         # print(depth_img.shape, type(depth_img))
    #         # PIL.Image.fromarray(depth_img).save(f'{outdir}/deep_seed{seed:04d}.png')
    ...
    #     # deep_img = torch.cat(depth_imgs, dim=2)

    #     PIL.Image.fromarray(img[0].cpu().numpy(), 'RGB').save(f'{outdir}/seed{seed:04d}.png')
    #     # PIL.Image.fromarray(deep_img[0].cpu().numpy()).save(f'{outdir}/deep_seed{seed:04d}.png')
    ...
    for i in range(1):
        print('Generating image for seed %d' % (i))
        ...
        for angle_y, angle_p in [(.4, angle_p), (0, angle_p), (-.4, angle_p)]:
            ...
            # c_samples_pose = LookAtPoseSampler.sample(np.pi/2, np.pi/2, cam_pivot, radius=cam_radius, device=device, horizontal_stddev=0.3, vertical_stddev=0.155)
            # c_samples = torch.cat([c_samples_pose.reshape(-1, 16), intrinsics.reshape(-1, 9)], 1)
            # print(c_samples)
            # cc = cam_sampler(1, device, cam_pivot, cam_radius, intrinsics)

            w_potential_path = f'/data1/sch/EG3D-projector-master/eg3d/projector_test_data/style_eg3d/emb/img00000219_w_plus.npy'
            ws = torch.from_numpy(np.load(w_potential_path)).to(device)
            img = G.synthesis(ws, camera_params)['image']

            img = (img.permute(0, 2, 3, 1) * 127.5 + 128).clamp(0, 255).to(torch.uint8)
            PIL.Image.fromarray(img[0].cpu().numpy(), 'RGB').save(f'{outdir}/seed{j}.png')
            j = j + 1
            imgs.append(img)
    ...
    #                 'G_ema': G.state_dict()
    #         }
    # checkpoint_path = f'/data1/sch/EG3D-projector-master/eg3d/networks/ffhq_stdcrop-128.pth'
