...
@click.option('--network', 'network_pkl', help='Network pickle filename', type=str, required=False, metavar='DIR', default='/data1/sch/EG3D-projector-master/eg3d/check/process_46/model_G_style.pth', show_default=True)
@click.option('--style_encoder', 'style_pkl', help='Network pickle filename', type=str, required=False, metavar='DIR', default='/data1/sch/EG3D-projector-master/eg3d/check/process_46/feature_match_final_16.pth', show_default=True)
@click.option('--style_img', 'Is_path', help='Network pickle filename', type=str, required=False, metavar='DIR', default='/data1/sch/EG3D-projector-master/eg3d/projector_test_data/style_eg3d/Pixar_FFHQ/pixar_generate_1880_01.png', show_default=True)
...
def generate_images(
    ...
    for angle_y, angle_p in [(.4, angle_p), (0, angle_p), (-.4, angle_p)]:
        ...
        w_potential_path = f'/data1/sch/EG3D-projector-master/eg3d/projector_test_data/style_eg3d/emb/img00000506_w_plus.npy'
        ...
        img = G.synthesis(w_c, camera_params, style_out=w_s, noise_mode='const')['image']
        # img = G.synthesis(w_c, c_samples, style_out=w_s, noise_mode='const')['image']

        img = (img.permute(0, 2, 3, 1) * 127.5 + 128).clamp(0, 255).to(torch.uint8)
        PIL.Image.fromarray(img[0].cpu().numpy(), 'RGB').save(f'{outdir}/506_style1880_{j}.png')
        j = j + 1
    ...
    PIL.Image.fromarray(img[0].cpu().numpy(), 'RGB').save(f'{outdir}/506_style1880.png')
