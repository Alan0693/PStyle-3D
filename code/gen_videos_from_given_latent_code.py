...
def gen_interp_video(G, latent, style_out, mp4: str,  w_frames=60 * 4, kind='cubic', grid_dims=(1, 1),
    ...
    _ = G.synthesis(ws[:1], c[:1], style_out=style_out)  # warm up
    ...
    for frame_idx in tqdm(range(num_keyframes * w_frames)):
        ...
        for yi in range(grid_h):
            for xi in range(grid_w):
                ...
                img = G.synthesis(ws=w.unsqueeze(0), c=c[0:1], style_out=style_out, noise_mode='const')[image_mode][0]
...
@click.command()
@click.option('--network', 'network_pkl', help='Network pickle filename', type=str, required=False, metavar='DIR', default='/data1/sch/EG3D-projector-master/eg3d/check/process_68/model_G_style.pth', show_default=True)
@click.option('--npy_path', 'npy_path', help='Network pickle filename', type=str, required=False, metavar='DIR', default='/data1/sch/EG3D-projector-master/eg3d/projector_test_data/style_eg3d/emb/img00000118_w_plus.npy', show_default=True)
@click.option('--style_encoder', 'style_pkl', help='Network pickle filename', type=str, required=False, metavar='DIR', default='/data1/sch/EG3D-projector-master/eg3d/check/process_68/feature_match_final_16.pth', show_default=True)
@click.option('--style_img', 'Is_path', help='Network pickle filename', type=str, required=False, metavar='DIR', default='/data1/sch/EG3D-projector-master/eg3d/projector_test_data/style_eg3d/Arance_FFHQ/arance_generate_1656.png', show_default=True)
...
@click.option('--outdir', help='Output directory', type=str, required=False, metavar='DIR', default='/data1/sch/EG3D-projector-master/out_style', show_default=True)
...
def generate_images(
    ...
    latent_s = Style_En(style_image)
    if latent_s.shape[1]!= G.backbone.mapping.num_ws:
        latent_s = latent_s.repeat([1, G.backbone.mapping.num_ws, 1])
    # latent_s = latent_s.repeat([1, G.backbone.mapping.num_ws, 1])

    # get cam_par
    # z_1 = torch.from_numpy(np.random.RandomState(1).randn(1, G.z_dim)).to(device)
    # z_2 = torch.from_numpy(np.random.RandomState(2).randn(1, G.z_dim)).to(device)
    # intrinsics = FOV_to_intrinsics(18.837, device=device)
    # cam_pivot = torch.tensor(G.rendering_kwargs.get('avg_camera_pivot', [0, 0, 0]), device=device)
    # cam_radius = G.rendering_kwargs.get('avg_camera_radius', 2.7)
    # conditioning_cam2world_pose = LookAtPoseSampler.sample(np.pi/2, np.pi/2, cam_pivot, radius=cam_radius, device=device)
    # conditioning_params = torch.cat([conditioning_cam2world_pose.reshape(-1, 16), intrinsics.reshape(-1, 9)], 1)
    # ws_1 = G.mapping(z_1, conditioning_params, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff)
    # ws_2 = G.mapping(z_2, conditioning_params, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff)

    add_weight = torch.ones(1, 14, 1).to(device)
    add_weight[:, 7:, :] = 0
    latent_cs = latent * add_weight + latent_s * (1 - add_weight)
    ...
    # gen_interp_video(G=G, latent = latent_cs, style_out=latent_s, mp4=output, bitrate='10M', grid_dims=grid, num_keyframes=num_keyframes,
    #                  w_frames=w_frames,  psi=truncation_psi,
    #                  truncation_cutoff=truncation_cutoff, cfg=cfg, image_mode=image_mode)

    # new_mixing
    gen_interp_video(G=G, latent = latent, style_out=latent_s, mp4=output, bitrate='10M', grid_dims=grid, num_keyframes=num_keyframes,
                     w_frames=w_frames,  psi=truncation_psi,
                     truncation_cutoff=truncation_cutoff, cfg=cfg, image_mode=image_mode)
