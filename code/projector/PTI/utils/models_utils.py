...
def load_D():
    with open(paths_config.eg3d_ffhq_pkl, 'rb') as f:
        ...
        D = network['D'].requires_grad_(False).to(global_config.device)
...
def load_FF_G(sampling_multiplier = 2):
    # with open(paths_config.stylegan2_ada_ffhq, 'rb') as f:
    #     old_G = pickle.load(f)['G_ema'].to(global_config.device).eval()
    #     old_G = old_G.float()
    # return old_G
    ...
    with open(paths_config.eg3d_ffhq_pkl, 'rb') as f:
        G = legacy.load_network_pkl(f)['G_ema'].to(global_config.device)  # type: ignore
        # network = legacy.load_network_pkl(f)
        # G = network['G_ema'].requires_grad_(False).to(global_config.device)
