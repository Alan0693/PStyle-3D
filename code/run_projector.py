...
from projector.PTI.utils.ImagesDataset import ImagesDataset
...
#               default='w_plus', show_default=True)
...
# # @click.option('--intial_w_path', help='intial w', type=str, required=True, metavar='STR', show_default=True)
...
@click.option('--network', 'network_pkl', help='Network pickle filename', required=False, 
                default='/data1/sch/EG3D-projector-master/eg3d/networks/ffhq512-128.pkl')
@click.option('--outdir', help='Output directory', type=str, required=False, metavar='DIR', default='projector_out')
@click.option('--latent_space_type', help='latent_space_type', type=click.Choice(['w', 'w_plus']), required=False, metavar='STR',
              default='w_plus', show_default=True)
@click.option('--image_path', help='image_path', type=str, required=False, metavar='STR', 
                default='/data1/sch/EG3D-projector-master/eg3d/projector_test_data/style_eg3d/cotent_data/', show_default=True)
@click.option('--c_path', help='camera parameters path', type=str, required=False, metavar='STR', 
                default='/data1/sch/EG3D-projector-master/eg3d/projector_test_data/style_eg3d/label', show_default=True)
@click.option('--sample_mult', 'sampling_multiplier', type=float,
              help='Multiplier for depth sampling in volume rendering', default=2, show_default=True)
@click.option('--num_steps', 'num_steps', type=int,
              help='Multiplier for depth sampling in volume rendering', default=500, show_default=True)
...
def run(
    ...
    # modify: 批处理
    dataset = ImagesDataset(image_path, transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])]))
    ...
    for fname, image in tqdm(dataloader):
        image_name = fname[0]
        # from_im = trans(image).cuda()
        id_image = torch.squeeze((image.cuda() + 1) / 2) * 255

        c_path_f = os.path.join(c_path,f'{image_name}.npy')
        c = np.load(c_path_f)
        c = np.reshape(c,(1,25))
        ...
        PTI_embedding_dir = f'/data1/sch/EG3D-projector-master/eg3d/projector_test_data/style_eg3d/embeddings/{image_name}'
        os.makedirs(PTI_embedding_dir,exist_ok=True)

        np.save(f'/data1/sch/EG3D-projector-master/eg3d/projector_test_data/style_eg3d/embeddings/{image_name}/{image_name}_{latent_space_type}.npy', w)
...
if __name__ == "__main__":
    ...
    # image_path = '/data1/sch/EG3D-projector-master/eg3d/projector_test_data/style_eg3d/cotent_data/'
    # em_path = '/data1/sch/EG3D-projector-master/eg3d/projector_test_data/style_eg3d/embeddings'
    # dataset = ImagesDataset(image_path, transforms.Compose([
    #     transforms.ToTensor(),
    #     transforms.Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])]))
    ...
    # for fname, image in tqdm(dataloader):
    #     image_name = fname[0]
    #     em_path_f = os.path.join(em_path,f'{image_name}/{image_name}_w_plus.npy')
    #     w = np.load(em_path_f)
    #     np.save(f'/data1/sch/EG3D-projector-master/eg3d/projector_test_data/style_eg3d/emb/{image_name}_w_plus.npy', w)
