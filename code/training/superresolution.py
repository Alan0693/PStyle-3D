...
class SuperresolutionHybrid8XDC(torch.nn.Module):
    ...
    def forward(self, rgb, x, ws, w_style=None, **block_kwargs):
        ws = ws[:, -1:, :].repeat(1, 3, 1)            # [1, 3, 512]
        # modify========================================
        wd = None
        wst = None
        if w_style != None:
            wd = w_style[:, -1:, :].repeat(1, 3, 1)
            wst = w_style[:, -1:, :].repeat(1, 2, 1)
        #===============================================
        ...
        # modify==================================================
        # Apply ModRes
        x, rgb = self.block0(x, rgb, ws, wst, wd, **block_kwargs)
        x, rgb = self.block1(x, rgb, ws, wst, wd, **block_kwargs)
        # No ModeRes
        # x, rgb = self.block0(x, rgb, ws, None, wd, **block_kwargs)
        # x, rgb = self.block1(x, rgb, ws, None, wd, **block_kwargs)
        #==========================================================
        return rgb
